...
@if ($paginator->hasPages())
    <ul class="inline-flex items-stretch -space-x-px">
        ...
        @if ($paginator->onFirstPage())
            <li>
                <!-- Disabled "Previous" link when on the first page -->
        ...
        @foreach ($elements as $element)
            ...
            @if (is_string($element))
                <li>
                    <a href="#" class="flex items-center justify-center text-sm py-2 px-3 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">{{ $element }}</a>
                </li>
            @endif
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <li>
                            <!-- Current Page -->
                            <a href="{{ $url }}" aria-current="page" class="flex items-center justify-center text-sm py-2 px-3 leading-tight text-primary-600 bg-primary-50 border border-primary-300 hover:bg-primary-100 hover:text-primary-700 dark:border-gray-700 dark:bg-gray-700 dark:text-white">{{ $page }}</a>
                        </li>
                    @else
                        <li>
                            <!-- Other Pages -->
        ...
        @else
            <li>
                <!-- Disabled "Next" link when on the last page -->
